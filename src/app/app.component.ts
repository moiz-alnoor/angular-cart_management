...
export class AppComponent implements OnInit {
  ...
  /* getProductsAsDefault  */
   getProducts() {
    this.productsParameters = {
      ...
      this.ProductsServiceService.getDefaultProducts(this.productsParameters).subscribe((productsData: any) => {
  ...
  /* getProductsAsDefault  */
   getRecipe(product:any) {
     ...
     for (let i = 0; i <  product.length; i++) {
      ...
      this.ProductsServiceService.getProductsRecipe(this.productsParameters).subscribe((recipeData: any) => {
  ...
  onChange(selectedOptions:any) {
      ...
      this.ProductsServiceService.selectedProducts(this.productsParameters).subscribe((productsData: any) => {
      this.products = productsData.products;
      this.isLoadingData  = false;
    });
  ...
  onAdd(product:any){
          this.exist = this.cartItems.find((x:any) => x.id == product.id);
              if (this.exist)  {
                  this.cartItems = this.cartItems.map((x:any) =>x.id === product.id ? { ...this.exist, qty: this.exist.qty + 1 } : x) ;
                } else {
